...
class RemoteConfigService: RemoteConfigurable{
    ...
    // Pertama, kita butuh satu variable untuk mengatur berapa interval fetch data
    // ke Firebase RemoteConfig dari local code kita
    ...
    // Fetch dan Activate configuration, simpah hasilnya sebagai status
    ...
    func fetchConfig<T>(forKey key: ConfigKeys) async throws -> T where T : Decodable {
        ...
        // Pastikan bahwa nilai dari config value yang kita set di Firebase remote config berupa String sesuai yang telah di set di Firebase
        ...
        do {
            ...
            throw
            ErrorConfiguration.failedToDecodeData(key: key.rawValue, underlyingError: error)
        }
...
enum ErrorConfiguration: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .failedToDecodeData(let key, let underlyingError):
            return "Failed to decode configuration for key: \(key). Errror: \(underlyingError.localizedDescription)"
